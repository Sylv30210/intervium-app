...
const visibility = `(entreprise_id = $1 AND (utilisateur_id = $2 OR (utilisateur_id IS NULL AND role_cible = $3)))`;
            ...
            await pool.query(
                `INSERT INTO notifications (entreprise_id, utilisateur_id, role_cible, type, titre, message, ressource_type, ressource_id, dedupe_key)
                 SELECT i.entreprise_id,
                        CASE WHEN $3 = 'TECHNICIEN' THEN $2 ELSE NULL END,
                        CASE WHEN $3 = 'ADMIN' THEN 'ADMIN' ELSE NULL END,
                 ...
                 WHERE i.entreprise_id = $1 AND i.statut = 'PLANIFIEE'
                   AND i.date_intervention BETWEEN CURRENT_DATE AND CURRENT_DATE + 2
                   AND ($3 = 'ADMIN' OR i.technicien_id = $2)
